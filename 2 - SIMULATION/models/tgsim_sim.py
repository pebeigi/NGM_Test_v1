"""TGSIM simulation runner (real-world fixed networks).

This module mirrors ``models.freeway_sim`` end-to-end: same CF/LC math
(IDM, PT-relative-IDM, MOBIL, DDM), same parameter sampling, same
communication bus and cooperative C-IDM/C-MOBIL, same TraCI loop, same
data-collection schema. Only the *geometry-dependent* parts are
different:

* The network template is used as-is (no procedural geometry / netconvert).
* Lane chains, ``pos_adjust_by_lanes`` and the MLC / lane-existence rules
  are written for the I90/94 diverge layout (templates/I90_94_simple.net.xml).
* There is no weaving area, so the reroute logic short-circuits via
  ``EXIT_DEFS = {}`` and ``_normalize_lane_seg`` is the identity.

To add another TGSIM network later, extend ``_resolve_tgsim_geometry``.
"""
...
def _resolve_tgsim_geometry(user_input_data):
    """Map TGSIM_Network -> full geometry description for a TGSIM scenario.

    Returns dict with:
        template:        absolute path to the .net.xml to copy in place
        basename:        short name for per-scenario sumocfg/net/rou files
        od_pairs:        list of (route_key_in_GUI, from_edge, to_edge, depart_lane)
                         where ``depart_lane`` is either an int (force this lane
                         index on the ``from_edge`` at departure) or the string
                         ``"best"`` (let SUMO pick a lane that fits the route).
        all_lane_names:  list[list[str]] giving the per-global-lane chain of
                         lane segment ids (Main -> internal -> branch)
        num_lanes_by_edge: dict edge_id -> SUMO lane count
        main_edge:       the single mainline edge (where MLC matters)
        branch_a_edge / branch_b_edge: the two diverge destinations
        branch_a_main_lanes / branch_b_main_lanes: sets of Main lane indices
                         that connect to each branch (used by the MLC logic)
        branch_a_internal_prefix / branch_b_internal_prefix: substrings used
                         to detect internal-edge segments per branch
    """
    network = (
        user_input_data.get("TGSIM_Network")
        or user_input_data.get("Geometry", {}).get("TGSIM_Network")
        or "I90/94"
    )

    if network == "I90/94":
        # I90_94_simple.net.xml:
        #   Main (6 lanes) -> Diverge -> NB (3 lanes), SB (3 lanes)
        #   Connections: Main_0/1/2 -> NB_0/1/2 via :Diverge_0_0/1/2
        #                Main_3/4/5 -> SB_0/1/2 via :Diverge_3_0/1/2
        all_lane_names = [
            ["Main_0", ":Diverge_0_0", "NB_0"],
            ["Main_1", ":Diverge_0_1", "NB_1"],
            ["Main_2", ":Diverge_0_2", "NB_2"],
            ["Main_3", ":Diverge_3_0", "SB_0"],
            ["Main_4", ":Diverge_3_1", "SB_1"],
            ["Main_5", ":Diverge_3_2", "SB_2"],
        ]
        # Lane-level OD: from each of the 6 Main start-lanes, separate volumes
        # to NB and SB. Cross-side trips (e.g. start in lane 0, destination SB)
        # are feasible — SUMO routes them and the MLC + boundary clamp + reroute
        # heuristics in run_sim_tgsim handle the weaving across Main.
        od_pairs = []
        for k in range(6):
            od_pairs.append((f"MainL{k}-NB", "Main", "NB", k))
            od_pairs.append((f"MainL{k}-SB", "Main", "SB", k))
        return {
            "template": str(TEMPLATES_DIR / "I90_94_simple.net.xml"),
            "basename": "i90_94",
            "od_pairs": od_pairs,
            "all_lane_names": all_lane_names,
            "num_lanes_by_edge": {"Main": 6, "NB": 3, "SB": 3},
            "main_edge": "Main",
            "branch_a_edge": "NB",
            "branch_b_edge": "SB",
            "branch_a_main_lanes": {0, 1, 2},
            "branch_b_main_lanes": {3, 4, 5},
            "branch_a_internal_prefix": ":Diverge_0",
            "branch_b_internal_prefix": ":Diverge_3",
        }

    raise ValueError(
        f"Unknown TGSIM network: {network!r}. "
        "Add a branch to models.tgsim_sim._resolve_tgsim_geometry()."
    )
...
# --- tgsim: hard Y-diverge reroute (Main -> branch_a OR branch_b) ------------
def tgsim_reroute(
    dest: str,
    edge: str,
    lane: str,
    ...
    branch_a_edge: str,
    branch_b_edge: str,
    branch_a_main_lanes: set,
    branch_b_main_lanes: set,
    branch_a_internal_prefix: str,
    branch_b_internal_prefix: str,
) -> tuple:
    """TGSIM hard-diverge reroute (two heuristics, mirrors ``freeway_reroute_on_off``).

    Heuristics:
      1. **infeasible / past_commit** — the vehicle's destination branch is no
         longer reachable from where it actually is. Either it's already on the
         wrong branch (past commit) or it's still on Main but too close to the
         gore to physically complete the required lane changes in the remaining
         distance.
      2. **sumo_waiting** — the vehicle has been stuck (SUMO waiting time *or*
         creeping below ``stuck_low_mps``) for at least ``stuck_time_req``
         seconds inside the diverge zone.

    On reroute we return the *reachable* branch as the new target, effectively
    accepting the missed exit so the vehicle proceeds instead of deadlocking at
    the gore (where SUMO would otherwise hold it for the rest of the sim).

    Returns ``(should_reroute, reason, wrong_lane, new_target)``. ``new_target``
    is empty when ``should_reroute`` is False.
    """
    if not dest:
        return (False, "", False, "")

    on_main = (edge == main_edge)
    on_branch_a = (edge == branch_a_edge) or (
        bool(branch_a_internal_prefix) and branch_a_internal_prefix in edge
    )
    on_branch_b = (edge == branch_b_edge) or (
        bool(branch_b_internal_prefix) and branch_b_internal_prefix in edge
    )

    # "Wrong lane" = destination branch is unreachable from current (edge, lane)
    # without crossing the hard diverge.
    wrong_for_a = (dest == branch_a_edge) and (
        (on_main and int(edge_lane_i) in branch_b_main_lanes) or on_branch_b
    )
    wrong_for_b = (dest == branch_b_edge) and (
        (on_main and int(edge_lane_i) in branch_a_main_lanes) or on_branch_a
    )
    wrong_lane = wrong_for_a or wrong_for_b

    if rerouted_f or not wrong_lane:
        return (False, "", wrong_lane, "")

    # Reachable branch given current position.
    if wrong_for_a:
        new_target = branch_b_edge
        valid_lanes = branch_a_main_lanes
    else:
        new_target = branch_a_edge
        valid_lanes = branch_b_main_lanes

    # Number of lane changes needed to reach the *correct* group from current
    # lane on Main (1 if not on Main — e.g. already on the wrong branch).
    if on_main and valid_lanes:
        n_lc = max(1, min(abs(int(edge_lane_i) - int(tl)) for tl in valid_lanes))
    else:
        n_lc = 1
    T_need = float(n_lc) * float(impos_sec_per_lc)
    v_now = float(v)
    ...
    should = False
    reason = ""
    # Rule 1a: past_commit — already on the wrong branch (no path back across gore).
    if on_branch_a or on_branch_b:
        should, reason = True, "past_commit"
    # Rule 1b: past_commit — past the end of Main but somehow still on Main.
    elif on_main and signed_d <= 0.0:
        should, reason = True, "past_commit"
    # Rule 1c: infeasible — can't physically finish required LCs in remaining distance.
    elif on_main and signed_d <= impos_lookahead and T_need > T_budget:
        should, reason = True, "infeasible"

    # Rule 2: sumo_waiting — stuck near the gore.
    in_zone = on_main or on_branch_a or on_branch_b
    _d_stuck = max(float(weave_reroute), float(impos_lookahead) * 0.5)
    near_stuck = (
        ((signed_d > 0) and (signed_d <= _d_stuck) and in_zone)
        or (near_end_of_current_edge and in_zone)
    )
    try:
        w = float(traci.vehicle.getWaitingTime(vid))
    except Exception:
    ...
    return (should, reason, wrong_lane, new_target if should else "")


def run_sim_tgsim(user_input_data, progress_cb=None, is_running_check=None):
    """Run a TGSIM scenario (real-world fixed network) and return per-step
    vehicle telemetry. Same DataFrame schema as ``run_sim_freeway``."""
    print(user_input_data)
    _geom = _resolve_tgsim_geometry(user_input_data)
    freeway_template_file = _geom["template"]
    basename = _geom["basename"]
    all_lane_names = _geom["all_lane_names"]
    num_lanes_by_edge = _geom["num_lanes_by_edge"]
    main_edge = _geom["main_edge"]
    branch_a_edge = _geom["branch_a_edge"]
    branch_b_edge = _geom["branch_b_edge"]
    branch_a_main_lanes = set(_geom["branch_a_main_lanes"])
    branch_b_main_lanes = set(_geom["branch_b_main_lanes"])
    branch_a_internal_prefix = _geom["branch_a_internal_prefix"]
    branch_b_internal_prefix = _geom["branch_b_internal_prefix"]

    # ``num_lanes`` and ``num_weave_lanes`` keep the same names freeway_sim.py
    # uses; TGSIM has no weaving so they're both the mainline lane count.
    num_lanes = int(num_lanes_by_edge[main_edge])
    num_weave_lanes = num_lanes

    # ``freeway_type`` is referenced by a few branches in code we copied
    # verbatim from freeway_sim.py. Pin it to a sentinel that doesn't match any
    # of the freeway_sim branches; the TGSIM-specific branches below check for
    # ``freeway_type == "tgsim"`` instead.
    freeway_type = "tgsim"

    # File paths for the (fixed) TGSIM network.
    new_file_name = str(MODELS_DIR / f"{basename}.net.xml")
    net_file_name = new_file_name
    route_file_name = str(MODELS_DIR / f"{basename}.rou.xml")
    config_file = str(MODELS_DIR / f"{basename}.sumocfg")

    # TGSIM templates ship with the real-world geometry already baked in. Just
    # copy the .net.xml in place; no netconvert step is needed.
    shutil.copyfile(freeway_template_file, new_file_name)
    ...
    pos_adjust_by_lanes = {}
    # TGSIM pos_adjust: every global lane is exactly Main -> internal -> branch.
    # Set the longitudinal offset so the global position runs continuously down
    # the trip: 0 on Main, len(Main) on the internal connector, len(Main) +
    # len(internal) on the branch. The lane_lengths dict is per *lane segment*
    # (TraCI lane id), so we look up the actual length for each segment used.
    for lane_chain in all_lane_names:
        cum = 0.0
        for seg in lane_chain:
            pos_adjust_by_lanes[seg] = cum
            cum += float(lane_lengths.get(seg, 0.0))

    # ---- TGSIM diverge bookkeeping --------------------------------------------
    # TGSIM has no weaving / off-ramps so we deliberately keep the freeway-style
    # EXIT_DEFS empty (the freeway reroute dispatch in the TraCI loop is gated
    # on it and naturally short-circuits). Instead we expose a single
    # ``DIVERGE_END_X`` — the longitudinal coordinate of the gore — that the
    # TGSIM reroute heuristics use to compute ``signed_d``.
    EXIT_DEFS = {}
    EXIT_X = 0.0
    EXIT_ORDER = []
    # All Main lanes share the same length (within numerical precision); pick
    # lane 0 as the canonical "end of Main / start of diverge" coordinate.
    DIVERGE_END_X = float(lane_lengths.get(f"{main_edge}_0", 0.0))

    def current_exit_key_for_position(lane_id, edge_id, global_pos):
        return None

    def exit_key_for_vehicle(dest, lane_id, edge_id, on_off1_side=False, on_off2_side=False):
        return None

    def in_exit_zone(exit_key, lane_id, edge_id):
        return False

    def in_mid_section(exit_key, lane_id):
        return False
    ...
    # TGSIM vehicle flows: keys match the GUI's volume page (setup_tgsim_inputs).
    # For I90/94 we have 12 lane-level OD pairs (MainL0-NB ... MainL5-SB).
    # Future TGSIM networks add their own keys via _resolve_tgsim_geometry.
    vf = user_input_data.get("Vehicle_Flows", {})
    in_flows = [float(vf.get(entry[0], 0.0)) for entry in _geom["od_pairs"]]
    ...
    def generate_agents(output_file="all_trips.trips.xml"):
        ...
        # OD pairs for TGSIM. Each entry: (route_key, in_edge, out_edge, depart_lane).
        # depart_lane is either an int (force a specific lane on in_edge) or the
        # string "best" (let SUMO pick a lane that fits the route).
        od_pairs = [
            (in_edge, out_edge, depart_lane, float(vf.get(route_key, 0.0)))
            for (route_key, in_edge, out_edge, depart_lane) in _geom["od_pairs"]
        ]

        depart_lanes_list = []
        for in_name, out_name, depart_lane, demand in od_pairs:
            if demand <= 0:
            ...
            # Store generation times keyed by (out_name, depart_lane) so multiple
            # OD entries with the same in/out but different starting lanes
            # (e.g. MainL0-NB vs MainL1-NB) don't overwrite each other.
            all_veh_generation_times[in_name][(out_name, depart_lane)] = generation_times

            for gen_t in generation_times:
                origins_list.append(in_name)
                destinations_list.append(out_name)
                depart_lanes_list.append(depart_lane)
                gen_times.append(gen_t)

        origins_arr=np.array(origins_list)
        destinations_arr=np.array(destinations_list)
        depart_lanes_arr = np.array(depart_lanes_list, dtype=object)
        ...
        depart_lanes_arr = depart_lanes_arr[sort_indices]
        ...
        for i in range(len(agent_ids)):
            ...
            # departLane: TGSIM's resolver supplies the desired lane index per
            # OD entry (so each Main start-lane is a separate trip in the routes
            # file). Numeric values are clamped to a string for SUMO; the
            # symbolic "best"/"random"/... fallbacks are also accepted in case a
            # future TGSIM network leaves a lane unspecified.
            _dl_raw = depart_lanes_arr[i]
            if isinstance(_dl_raw, (int, np.integer)):
                _depart_lane = str(int(_dl_raw))
            else:
                _depart_lane = str(_dl_raw).strip()
    ...
    def _normalize_lane_seg(lane_seg):
        """TGSIM identity normalizer.

        The freeway runner remaps connector-edge variants (e.g. ":Weaving_Start_1_-1"
        -> ":Weaving_Start_0_0") because lane indices on internal edges sometimes
        appear with negative suffixes. TGSIM networks ship with explicit lane
        chains in ``all_lane_names`` and have no such aliasing, so the identity
        is sufficient — we just gate on whether the segment is one we know about.
        """
        if lane_seg is None:
            return None
        lane_seg = str(lane_seg)
        if lane_seg not in lane_segment_to_lane_list:
            return None
        return lane_seg
    ...
    while t < sim_time:
        ...
        for i_id in range(len(all_ids)):
            ...
            # TGSIM: hard diverge, no weaving. ``wrong_lane`` is always False so
            # the reroute branch below is skipped. We compute side flags that the
            # MLC block further down uses to push vehicles toward the right branch.
            on_offramp_side = False
            on_mainline_side = False
            on_offramp1_side = False
            on_offramp2_side = False
            on_mainline_side1 = False
            on_mainline_side2 = False
            wrong_exit_lane = False
            wrong_exit_lane1 = False
            wrong_exit_lane2 = False
            wrong_through_lane = False
            wrong_lane = False

            # For TGSIM destination = the branch edge (e.g. "NB" / "SB").
            on_branch_a = (edge == branch_a_edge) or (branch_a_internal_prefix in edge)
            on_branch_b = (edge == branch_b_edge) or (branch_b_internal_prefix in edge)
            ...
            # --- TGSIM diverge reroute (2 heuristics: infeasible/past_commit + sumo_waiting) ---
            # Mirrors freeway_reroute_on_off: when a vehicle is in a wrong lane
            # group near the gore (or has been stuck for STUCK_TIME_REQUIRED s),
            # switch its destination to the *reachable* branch so it doesn't
            # deadlock at the diverge. Only one reroute per vehicle.
            if vid not in rerouted_flag:
                rerouted_flag[vid] = False
            if (not rerouted_flag[vid]) and (DIVERGE_END_X > 0.0):
                _signed_d_t = DIVERGE_END_X - float(global_pos)
                _edge_lane_i_t = int(edge_lane_indexes[i_id])
                (_should_t, _reason_t, _wrong_t, _new_target_t) = tgsim_reroute(
                    dest,
                    edge,
                    lane,
                    _edge_lane_i_t,
                    ...
                    branch_a_main_lanes,
                    branch_b_main_lanes,
                    branch_a_internal_prefix,
                    branch_b_internal_prefix,
                )
                if _should_t and _new_target_t:
                    if REROUTE_DEBUG:
                        try:
                            _wt_t = float(traci.vehicle.getWaitingTime(vid))
                        except Exception:
                            _wt_t = 0.0
                        print(
                            f"[TGSIM REROUTE] vid={vid} dest={dest} -> {_new_target_t} "
                            f"reason={_reason_t} edge={edge} lane={lane} laneI={_edge_lane_i_t} "
                            f"d={_signed_d_t:.1f} wait={_wt_t:.1f}"
                        )
                    ok = safe_change_target(vid, _new_target_t)
                    if ok:
                        veh_destinations_by_id[vid] = _new_target_t
                        dest = _new_target_t  # sync local copy so MLC below uses the new dest
                        rerouted_flag[vid] = True
                    else:
                        # findRoute failed (e.g. already past the gore onto the wrong branch
                        # with no path back). Stop retrying every step to avoid TraCI spam.
                        rerouted_flag[vid] = True
            ...
            # --- Mandatory LC (MOBIL/DDM) for TGSIM: push vehicles toward the
            # lane group that physically connects to their destination branch.
            # For I90/94: NB connects from Main lanes 0-2; SB from lanes 3-5.
            if edge == main_edge:
                if dest == branch_a_edge and lane_i not in branch_a_main_lanes:
                    # On Main but not yet on a lane that goes to branch_a -> move right.
                    MLC_right = 1
                elif dest == branch_b_edge and lane_i not in branch_b_main_lanes:
                    # On Main but not yet on a lane that goes to branch_b -> move left.
                    MLC_left = 1
            ...
            # TGSIM: left/right lane existence is just bounded by the per-edge
            # lane count (the freeway runner has extra weaving-specific clamps;
            # we have no weaving). ``right_lane_exists`` is already set above
            # iff lane_i >= 1; here we add the symmetric ``left_lane_exists``
            # using the per-edge maximum lane index.
            _edge_lanes = int(num_lanes_by_edge.get(edge, num_lanes))
            left_lane_exists = 0
            if lane_i < (_edge_lanes - 1) and MLC_right == 0:
                left_lane_exists = 1

            # TGSIM boundary clamp: on Main, never LC across the branch-group
            # boundary (e.g. a NB-bound vehicle in lane 2 must not drift to
            # lane 3, which only connects to SB). This prevents the most common
            # cause of stuck vehicles at the diverge.
            if edge == main_edge:
                if dest == branch_a_edge:
                    if (lane_i + 1) in branch_b_main_lanes:
                        left_lane_exists = 0
                    if (lane_i - 1) in branch_b_main_lanes:
                        right_lane_exists = 0
                elif dest == branch_b_edge:
                    if (lane_i + 1) in branch_a_main_lanes:
                        left_lane_exists = 0
                    if (lane_i - 1) in branch_a_main_lanes:
                        right_lane_exists = 0
