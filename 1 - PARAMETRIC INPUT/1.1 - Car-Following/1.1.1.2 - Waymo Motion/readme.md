...
| File | Description |
|------|-------------|
| `Waymo_IDM_CF_Calibration.py` | Calibrates the **Intelligent Driver Model (IDM)** on pre-processed Waymo leader–follower trajectories with a genetic algorithm (40 population × 80 generations). Groups events into `Waymo_S` (length < 6 m), `Waymo_L` (≥ 6 m), and `Waymo_A` (self-driving car, `is_sdc`); skips segments shorter than 15 s. Fits `(T, a, b, v0, s0, δ)` per event, simulates, and scores against observed kinematics. CLI flags include `--max-events`, `--plot-only`, `--summary-only`. |
...
|-----------------------------------|-------------|
| `March2023waymo_scenario_lane_leader_follower_assigned_{196–199}_data.csv` | Per-timestep vehicle tracks with assigned lane, leader/follower IDs, SDC flag, position and velocity. Main calibration input. |
| `March2023waymo_map_features_{196–199}_data.csv` | Per-scenario map geometry: lane polylines, road edges/lines, crosswalks, speed limits. Used for visualization and diagnostics. |
